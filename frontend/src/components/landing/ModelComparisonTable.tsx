...
const FLAGSHIP_IDS = [
  'gpt-5', 'claude-opus-4-6', 'gemini-3-1-pro', 'o3',
  'grok-4', 'llama-4-maverick', 'deepseek-r1', 'mistral-large-3',
];
...
export default function ModelComparisonTable() {
  ...
  // Prefer known flagship IDs; fall back to top-rated models
  let flagships = models.filter((m) => FLAGSHIP_IDS.includes(m.id));
  if (flagships.length < 4) {
    flagships = [...models].sort((a, b) => b.rating - a.rating).slice(0, 8);
  }
